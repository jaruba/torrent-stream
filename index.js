...
var torrentStream = function (link, opts, cb) {
  ...
  var fastResumeDataPath = path.join((opts.resumeDataFolder || opts.tmp), infoHash + '.fastresume');
...
//          discovery.restart();
    ...
    var verify = function(shouldRewriteFastResume) {

      var loop = function(i) {
        if (i >= torrent.pieces.length) {
          if (shouldRewriteFastResume) {
    ...
    if (!opts.fastresume) {
      ...
      fs.readFile(fastResumeDataPath, function(err, buf) {
        if (err) return verify(true)

        // 20 bytes is the size of SHA1 hash
        var fastResumeDataHash = sha1(buf.slice(20), true)

        if (!buf.slice(0, 20).equals(fastResumeDataHash)) return verify(true)

        if (engine.bitfield.buffer.length !== buf.slice(20).length) return verify(true)
